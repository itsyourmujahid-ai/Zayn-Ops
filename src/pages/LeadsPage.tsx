...
import { subscribeToLeads, getAllUsers, getUserDisplayName, deleteLead } from '../lib/dal';
...
export const LeadsPage: React.FC<LeadsPageProps> = ({
  ...
  const [deleteTargetLead, setDeleteTargetLead] = useState<LeadRecord | null>(null);
  const [deleteReasonText, setDeleteReasonText] = useState<string>('Lead deletion by administrator');
  ...
  const handleExecuteAdminDelete = async () => {
    if (!isAdmin) return;
    const leadsToDelete = deleteTargetLead
      ? [deleteTargetLead.id]
    ...
    try {
      const actor = {
        ...
        name: userProfile?.full_name || currentUser?.email || 'Administrator',
        role: userProfile?.role || 'ADMIN',
        company_id: userProfile?.company_id,
      };

      for (const leadId of leadsToDelete) {
        await deleteLead(leadId, deleteReasonText.trim() || 'Lead deletion by administrator', actor);
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-xs overflow-hidden">
        {/* Table Selection Action Banner */}
        {selectedLeadIds.size > 0 && (
          <div className="flex flex-wrap items-center justify-between gap-3 bg-indigo-50 border-b border-indigo-100 px-4 py-2.5">
            <div className="flex items-center gap-2">
              <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-indigo-600 text-xs font-black text-white">
            ...
            {isAdmin && (
              <button
                id="admin-delete-selected-leads-btn"
                type="button"
                onClick={() => {
                  setDeleteTargetLead(null);
                  setDeleteReasonText('Bulk lead deletion by administrator');
                  setDeleteModalOpen(true);
                }}
                className="inline-flex items-center gap-1.5 rounded-lg bg-rose-600 hover:bg-rose-700 px-3.5 py-1.5 text-xs font-bold text-white shadow-xs transition cursor-pointer"
                            ...
                            {isAdmin && (
                              <button
                                ...
                                onClick={(e) => {
                                  ...
                                  setDeleteReasonText('Lead deletion by administrator');
                                  setDeleteModalOpen(true);
                                }}
                                className="rounded-lg bg-rose-50 p-1.5 text-rose-600 hover:bg-rose-100 hover:text-rose-700 transition shadow-2xs cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 animate-in fade-in duration-150">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2 text-rose-600">
                ...
                <h3 className="text-base font-bold text-slate-900">
                  {deleteTargetLead
                    ? 'Confirm Delete Lead'
                    : `Confirm Delete ${selectedLeadIds.size} Leads`}
                </h3>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Duplicate record, Invalid contact, Opportunity closed"
              ...
              <div className="rounded-lg bg-amber-50 border border-amber-200 p-2.5 text-[11px] text-amber-800 flex items-start gap-2">
                ...
                <span>
                  All activities, follow-ups, and customer relationship links are archived and audited in compliance with CRM integrity rules.
                </span>
              </div>
            </div>
            ...
            <div className="mt-6 flex items-center justify-end gap-2 border-t border-slate-100 pt-3">
              ...
              <button
                type="button"
                id="btn-confirm-admin-delete"
                disabled={isDeleting}
                onClick={handleExecuteAdminDelete}
                className="inline-flex items-center gap-1.5 rounded-xl bg-rose-600 hover:bg-rose-700 px-4 py-2 text-xs font-bold text-white shadow-xs transition cursor-pointer disabled:opacity-50"
              >
                {isDeleting ? (
                  <>
                  ...
                  <>
                    ...
                    <span>Delete {deleteTargetLead ? 'Lead' : `(${selectedLeadIds.size})`}</span>
                  </>
                )}
              </button>
            </div>
